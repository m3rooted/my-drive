...
const sendScheduleReminderEmail = async (email, schedule, minutesBefore, dueAt) => {
  if (!isEmailConfigured()) {
  ...
  const dueTime = formatDateTime(dueAt || schedule.endAt || schedule.startAt);

  try {
    await transporter.sendMail({
      from,
      to: email,
      subject: `Reminder: ${schedule.title} is due soon`,
      text: [
        `Your schedule item is due in about ${minutesBefore} minutes.`,
      ...
      html: `
        <div style="font-family: Arial, sans-serif; line-height: 1.5; color: #1f2937;">
          ...
          <p>Your schedule item is due in about <strong>${minutesBefore} minutes</strong>.</p>
